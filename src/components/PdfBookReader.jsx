...
export default function PdfBookReader({ data, initialLocation, jump, onLocation }) {
  ...
  return <div className="pdf-reader" ref={container}>
    <div className="digital-toolbar">
      <button className="btn btn-secondary" disabled={!document || page <= 1} onClick={() => setPage(p => p - 1)}>Previous page</button>
      ...
      <button className="btn btn-secondary" disabled={!document || page >= document.numPages} onClick={() => setPage(p => p + 1)}>Next page</button>
    ...
    <div className="pdf-page"><canvas ref={canvas} aria-label={`PDF page ${page}`}/></div>
