...
function ReaderSession({ file, userId }) {
  ...
  return <>
    <Suspense fallback={<p role="status">Preparing reader…</p>}><Viewer {...loaded} jump={jump} onLocation={onLocation}/></Suspense>
...
export default function DigitalBookReader({ bookId, userId }) {
  ...
  async function remove() {
    if (!file || !confirm(`Remove ${file.filename} and its saved place and bookmarks? Your reading notes will remain.`)) return
    setBusy(true)
  ...
  return <section className="card digital-book-reader" aria-label="Digital book reader">
    ...
    {error ? <div role="alert"><p>{error}</p><button className="btn btn-secondary" onClick={() => setAttempt(n => n + 1)}>Retry</button></div> : files === null ? <p role="status">Loading book files…</p> : !files.length ? <p>Add a digital copy to read here. Your reading notes are below.</p> : <>
      ...
      {file && <ReaderSession key={file.id} file={file} userId={userId}/>}
    </>}
  </section>
}
